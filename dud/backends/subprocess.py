...
The Session is the host half: it owns the socketpair, answers the
guest's reverse requests (cache reads, hostcalls, emits), and applies
cache write-backs from successful execs. Cache values are opaque
pickle bytes end to end — this backend never unpickles them.
"""
...
class Session:
    ...
    - ``host_objects``: name -> live object. Guests reach them solely
      via hostcall; ``allow`` maps name -> iterable of permitted method
      names (default: all public callables — rung-1 cooperative
      posture; tighten at real boundaries).
    - ``on_emit``: callback(name, value) for guest emits; also
      collected in ``self.emits``.
    """
